...
4. 已共享路径点名称前显示锁图标，共享和编辑按钮变灰；如需移除，点击仍可用的 `删除共享`。

创建者客户端直接复用所选的 Xaero 路径点，不会再创建一份本地副本。其他有权限的客户端会收到服务器同步的 Xaero 路径点副本。路径点一经共享即不可修改，所有客户端都会恢复服务器版本；删除仍由创建者或服务端管理员执行。公开和队伍可见性、创建者身份、revision 与删除 tombstone 均由服务器校验和维护。
...
- 所有玩家都不能修改已共享路径点；创建者可以删除自己的共享路径点，OP 可通过服务端命令删除。
